<template>
  <view id="app">
    ...
    <view id="__next">
      <view class="overflow-hidden w-full h-full relative">
        <view class="flex h-full flex-1 flex-col md:pl-[260px]">
          <view
          ...
          <main class="relative h-full w-full transition-width flex flex-col overflow-hidden items-stretch flex-1">
            ...
            <view class="flex-1 overflow-hidden">
              <view class="react-scroll-to-bottom--css-ncqif-79elbk h-full dark:bg-gray-800">
                <view ref="chatContainer" class="react-scroll-to-bottom--css-krija-1n7m0yu">
                  <view class="flex flex-col items-center text-sm dark:bg-gray-800">
                    <!-- 对话item -->
                    <template v-for="conv, idx in conversation">
                      <!-- human -->
                      <view v-if="conv.speaker == 'human'"
                        ...
                        <view
                          ...
                          <view class="w-[30px] flex flex-col relative items-end">
                          ...
                          <view class="relative flex w-[calc(100%-50px)] flex-col gap-1 md:gap-3 lg:w-[calc(100%-115px)]">
                            <view class="flex flex-grow flex-col gap-3">
                              <view class="min-h-[20px] flex flex-col items-start gap-4 whitespace-pre-wrap">{{
                                conv.speech
                              }}
                              </view>
                            </view>
                      ...
                      <view v-if="conv.speaker == 'ai'"
                        ...
                        <view
                          ...
                          <view class="w-[30px] flex flex-col relative items-end">
                            ...
                            <!-- 多个消息 -->
                            <view v-if="conv.speeches.length > 1"
                          ...
                          <view class="relative flex w-[calc(100%-50px)] flex-col gap-1 md:gap-3 lg:w-[calc(100%-115px)]">
                            <view class="flex flex-grow flex-col gap-3">
                              <!--  whitespace-pre-wrap -->
                              <view class="min-h-[20px] flex flex-col items-start gap-4">
                                <view v-html="mdToHtml(conv.speeches[conv.idx], conv)"
                    ...
                    <view v-if="conversation.length == 0"
                      ...
                      <h1
                        ...
                        ChatGPT</h1>
                      <view class="md:flex items-start text-center gap-3.5">
                        <view class="flex flex-col mb-8 md:mb-auto gap-3.5 flex-1">
                    ...
                    <view class="w-full h-32 md:h-48 flex-shrink-0"></view>
                  </view>
            ...
            <view
              ...
              <form class="stretch mx-2 flex flex-row gap-3 pt-2 last:mb-2 md:last:mb-6 lg:mx-auto lg:max-w-3xl lg:pt-6">
                <view class="relative flex h-full flex-1 md:flex-col">
                  <view class="flex ml-1 md:w-full md:m-auto md:mb-2 gap-0 md:gap-2 justify-center">
                  ...
                  <view
                    ...
                    <button @click.stop.prevent="send" :disabled="convLoading"
                      ...
                      <view v-if="convLoading" class="text-2xl" style="line-height: 1.3rem;">
              ...
              <view class="px-3 pt-2 pb-3 text-center text-xs text-black/50 dark:text-white/50 md:px-4 md:pt-3 md:pb-6">
        ...
        <!-- 菜单导航 -->
        <view class="dark hidden bg-gray-900 md:fixed md:inset-y-0 md:flex md:w-[260px] md:flex-col">
          <view class="flex h-full min-h-0 flex-col ">
            <view ref="menu" class="scrollbar-trigger flex h-full w-full flex-1 items-start border-white/20">
              ...
              <nav ref="navEle" class="flex h-full flex-1 flex-col space-y-1 p-2">
                ...
                <!-- 对话列表 -->
                <view class="flex-col flex-1 overflow-y-auto border-b border-white/20" style="padding-bottom: 5px;">
                  <view class="flex flex-col gap-2 text-gray-100 text-sm">

                    <template v-for="conversation, cidx in conversations">

                      <view v-if="conversation.editable"
                        ...
                        <view class="absolute flex right-1 z-10 text-gray-300 visible">
                      ...
                      <a v-else-if="conversation.delete" @blur="cancelDelConv(cidx, conversation)"
                        ...
                        <view class="flex-1 text-ellipsis max-h-5 overflow-hidden break-all relative">Delete "{{
                          conversation.title
                        }}"?
                          <view class="absolute inset-y-0 right-0 w-8 z-10 bg-gradient-to-l from-gray-800"></view>
                        </view>
                        <view class="absolute flex right-1 z-10 text-gray-300 visible">
                      ...
                      <a v-else @click.stop.prevent="selectConversation(conversation, true)"
                        ...
                        <view class="flex-1 text-ellipsis max-h-5 overflow-hidden break-all relative">
                          {{ conversation.title }}
                          <view
                        ...
                        <view v-show="conversation.selected" class="absolute flex right-1 z-10 text-gray-300 visible">
      ...
      <view class="absolute top-0 left-0 right-0 z-[2]"></view>
    </view>

    <view v-show="showSlide" class="semi-portal" style="z-index: 1000;">
      <view class="">
        <view class="semi-modal-mask"></view>
        <view role="none" class="semi-modal-wrap">
          <view class="semi-modal semi-modal-small" id="dialog-3" style="width: 0px;">
            <view role="dialog" aria-modal="true" aria-labelledby="semi-modal-title" aria-describedby="semi-modal-body"
              class="semi-modal-content">
              <view class="semi-modal-body-wrapper">
                <view class="semi-modal-body" x-semi-prop="children">
                  <view class="fixed inset-0 z-40 flex">
                    <view class="relative flex w-full max-w-xs flex-1 flex-col bg-gray-900 translate-x-0"
                      id="headlessui-dialog-panel-:r1:" data-headlessui-state="open">
                      <view class="absolute top-0 right-0 -mr-12 pt-2 opacity-100">
                      ...
                      <view ref="slideNavContainer" style="width:320px"
                        class="flex h-full flex-1 items-start border-white/20">


                      </view>
                    </view>
                    <view @click="closeShowSlide" style="width:calc(100% - 320px)" class="flex-shrink-0"></view>
                  </view>
                </view>
              </view>
            </view>
    ...
    <!-- 弹窗 -->
    <view id="headlessui-portal-root" v-if="popupShow">
      <view data-headlessui-portal="">
        ...
        <view>
          <view class="relative z-50" id="headlessui-dialog-:r3:" role="dialog" aria-modal="true"
            data-headlessui-state="open" aria-labelledby="headlessui-dialog-title-:r5:">
            <view class="fixed inset-0 bg-gray-500/90 transition-opacity dark:bg-gray-800/90"></view>
            <view class="fixed inset-0 z-50 overflow-y-auto">
              <view class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                ...
                <view v-if="popupShow"
                  ...
                  id="headlessui-dialog-panel-:r1:" data-headlessui-state="open">
                  <view class="flex items-center sm:flex">
                    <view class="mt-3 text-center sm:mt-0 sm:text-left"></view>
                  </view>
                  <view class="prose dark:prose-invert">
                    <view class="mb-5">
                      <h2 class="!mt-4 font-normal !mb-2"><b>ChatGPT</b></h2>
                    </view>
                    <view class="w-full h-[1px] bg-gray-300 opacity-20"></view>
                    <h4 class="mb-4">
          `            本开源项目基于openai开放api开发，使用最新的gpt-3.5-turbo，前端使用vue高仿ChatGpt界面，仅供学习使用.</h4>
                    <view class="flex gap-4 flex-col text-sm">
                      <view class="flex p-4 bg-gray-50 dark:bg-white/5 rounded-md items-center gap-4 min-h-[71px]">
                        <view class="w-10 text-2xl text-center">🚨</view>
                        <view class="flex-1 leading-5">
                          本开源项目有搭建教程，如有能力可以自行搭建，因为openai的api是付费的，项目不会长期提供演示。（用完官网送的18美元，就会停止项目演示）
                        </view>
                      </view>
                      <view class="flex p-4 bg-gray-50 dark:bg-white/5 rounded-md items-center gap-4 min-h-[71px]">
                        <view class="w-10 text-2xl text-center">🔬</view>
                        <view class="flex-1 leading-5">随着使用人数的增多和api被墙等原因，演示环境过几天就会关闭问答功能。如果需要长期使用，可以联系我，微信：zjb592466695
                        </view>
                      </view>
                    </view>
                  ...
                  <view class="mt-5 flex flex-col gap-3 sm:mt-4 sm:flex-row-reverse"></view>
                </view>
                ...
                <view v-if="false"
                  ...
                  <view class="flex items-center sm:flex">
                    ...
                    <view class="mt-3 text-center sm:mt-0 sm:text-left">
                  ...
                  <view class="mt-5 flex flex-col gap-3 sm:mt-4 sm:flex-row-reverse">
...
const renderer = {
  code(code, infostring, escaped) {
    ...
    return `<view class="bg-black mb-4 rounded-md">
      <view class="code_header flex items-center relative text-gray-200 bg-gray-800 px-4 py-2 text-xs font-sans">
...
export default {
  ...
  methods: {
    ...
    isScrollAndNotBottom() {
      let chatviewEle = this.$refs.chatContainer;
      if (!chatviewEle) {
        return;
      }

      if (chatviewEle.scrollHeight <= chatviewEle.clientHeight) {
        this.isShowGoBottom = false;
        return;
      }

      const scrollTop = chatviewEle.scrollTop;
      const windowHeight = chatviewEle.clientHeight;
      const scrollHeight = chatviewEle.scrollHeight;
  ...
  mounted: function () {
    ...
    let chatviewEle = this.$refs.chatContainer;
    chatviewEle.addEventListener('scroll', this.isScrollAndNotBottom, true)
